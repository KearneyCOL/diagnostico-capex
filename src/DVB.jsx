...
export default function DVB() {
  ...
  useEffect(() => {
    ...
    if (urlId) {
      ...
      loadAssessment(urlId)
        .then(data => {
          if (data) { setAns(data); }
            ...
            try { const s = localStorage.getItem(STORAGE_KEY); if (s) setAns(JSON.parse(s)); } catch {}
          }
        })
        .catch(() => {
          try { const s = localStorage.getItem(STORAGE_KEY); if (s) setAns(JSON.parse(s)); } catch {}
      ...
      // Sin URL — muestra modal para elegir ID
      try { const s = localStorage.getItem(STORAGE_KEY); if (s) setAns(JSON.parse(s)); } catch {}
      setShowModal(true);
      setHydrated(true);
    }
  ...
  // Guarda en localStorage inmediatamente + Supabase con debounce 1.5s
  useEffect(() => {
    if (!hydrated) return;
    ...
    saveTimer.current = setTimeout(async () => {
      try {
        await saveAssessment(assessId, ans);
        setSaveStatus("saved");
        setTimeout(() => setSaveStatus("idle"), 2500);
      } catch { setSaveStatus("error"); }
    }, 1500);
    return () => clearTimeout(saveTimer.current);
  }, [ans, hydrated, assessId]);
  ...
  const TABS = [{k:"intro",l:"Introducción"},{k:"detail",l:"Diagnóstico"},{k:"heatmap",l:"Heatmap"},{k:"resumen",l:"Resumen"}];
...
const resetAll = () => {
  ...
  const o = emptyAns();
  setAns(o);
  // También sobreescribe en Supabase
  if (assessId) saveAssessment(assessId, o).catch(()=>{});
  setTab("intro");
};
